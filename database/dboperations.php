...
class DBOperations
{
    ...
    {
        return getAll("
            select p.id_product, p.image, p.product_name, p.price, p.amount, p.year, pt.product_type_name, ct.country_name, p.product_type_id from products p 
    ...
    {
        return getOne("
            select p.id_product, p.image, p.product_name, p.price, p.amount, p.year, pt.product_type_name, ct.country_name from products p 
            left join product_types pt on pt.id_product_type = p.product_type_id
            left join countries ct on ct.id_country = p.country_id
     ...
     public function addProductInCart($count, $product_id, $user_id)
     {
         ...
         $product_isset = getOne("
             select * from cart_products where product_id = '$product_id' and user_id = '$user_id'
         ");
         ...
         if ($product_isset) {
             ...
             $success = operation("
                 update cart_products set count = $count where id_cart_product = '$new_id'
             ");
    ...
    {
        return getAll("
            select cp.id_cart_product, cp.count, p.id_product, p.image, p.product_name, p.price, p.year, pt.product_type_name, ct.country_name from cart_products cp 
    ...
    {
        return getAll("
            select p.id_product, p.image, p.product_name, p.price, p.amount, p.year, p.product_type_id, p.country_id, pt.product_type_name, ct.country_name, p.product_type_id from products p 
            left join product_types pt on pt.id_product_type = p.product_type_id
            left join countries ct on ct.id_country = p.country_id
            order by p.id_product desc;
        ");
    ...
    public function updateProduct($product_id, $product_name, $price, $amount, $year, $product_type_id, $country_id)
    {
        $success = operation("
            update products set 
